...
<?php 
    ...
    class LoginValidator{
        ...
        public function addUser(String $user, String $passw){
            try{
                $this->con->beginTransaction();
                $query = $this->con->prepare("insert into login (user, userPassw) 
                VALUES (?, ?)");
                $query->execute([$user, $passw]);
                $numberOfAddedRows = $query->rowCount();
                $this->con->commit();
                return $numberOfAddedRows;
        ...
        public function validLogin(String $user, String $passw){
            try{
